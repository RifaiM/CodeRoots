...
function checkAnswer() {
    ...
    // More comprehensive regex patterns for mathematical operations
    const variableWithOperator = (code.match(/[a-zA-Z_$][a-zA-Z0-9_$]*\s*[\+\-\*\/\%]\s*[a-zA-Z0-9_$\s]+/g) || []).length;
    const numberWithOperator = (code.match(/\d+\s*[\+\-\*\/\%]\s*\d+/g) || []).length;
    const operatorWithVariable = (code.match(/\d+\s*[\+\-\*\/\%]\s*[a-zA-Z_$][a-zA-Z0-9_$]*/g) || []).length;
    
    const meaningfulMath = variableWithOperator + numberWithOperator + operatorWithVariable;
