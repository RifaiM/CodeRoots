 ...
 */
/**
 * Checks for basic HTML structure requirements (simplified for educational context)
 * @param {string} code - The HTML code to validate
 * @returns {Array} Array of error messages for missing requirements
 */
function checkBasicHTMLStructure(code) {
  const errors = [];
  
  // Remove comments to avoid false positives
  let cleanCode = code.replace(/\/\*[\s\S]*?\*\//g, '').replace(/<!--[\s\S]*?-->/g, '');
  
  // Check for basic structure requirements (more lenient approach)
  const hasHtmlTag = /<html/i.test(cleanCode) && /<\/html>/i.test(cleanCode);
  const hasHeadSection = /<head/i.test(cleanCode) && /<\/head>/i.test(cleanCode);
  const hasBodySection = /<body/i.test(cleanCode) && /<\/body>/i.test(cleanCode);
  const hasNavElement = /<nav/i.test(cleanCode) && /<\/nav>/i.test(cleanCode);
  
  // Only check for tags that are commonly expected in this lesson
  if (!hasHtmlTag) {
    errors.push('HTML document should have &lt;html&gt; and &lt;/html&gt; tags');
  }
  
  if (!hasHeadSection) {
    errors.push('HTML document should have &lt;head&gt; and &lt;/head&gt; tags');
  }
  
  if (!hasBodySection) {
    errors.push('HTML document should have &lt;body&gt; and &lt;/body&gt; tags');
  }
  
  if (!hasNavElement) {
    errors.push('Navigation should use &lt;nav&gt; and &lt;/nav&gt; tags');
  }
  
  // Check for common unclosed tag issues (more specific)
  const checkSpecificTags = [
    { name: 'ul', pattern: /<ul[^>]*>[\s\S]*?<\/ul>/gi },
    { name: 'li', open: /<li[^>]*>/gi, close: /<\/li>/gi },
    { name: 'a', open: /<a[^>]*>/gi, close: /<\/a>/gi }
  ];
  
  checkSpecificTags.forEach(tag => {
    if (tag.pattern) {
      // Check if ul elements are properly closed
      const openULs = (cleanCode.match(/<ul[^>]*>/gi) || []).length;
      const completeULs = (cleanCode.match(tag.pattern) || []).length;
      if (openULs > completeULs) {
        errors.push(`Found unclosed &lt;${tag.name}&gt; tags - make sure each &lt;${tag.name}&gt; has a matching &lt;/${tag.name}&gt;`);
      }
    } else {
      // Check opening vs closing tags
      const openTags = (cleanCode.match(tag.open) || []).length;
      const closeTags = (cleanCode.match(tag.close) || []).length;
      
      if (openTags > 0 && closeTags < openTags) {
        const missing = openTags - closeTags;
        errors.push(`Missing ${missing} &lt;/${tag.name}&gt; closing tag${missing > 1 ? 's' : ''}`);
      }
    }
  });
...
function checkClosingTags(code) {
  return checkBasicHTMLStructure(code);
}
