...
function checkAllSemicolons(code) {
    const errors = [];
    const warnings = [];// Check JavaScript semicolons
    const jsResults = checkJavaScriptSemicolons(code);
    errors.push(...jsResults.errors);
    warnings.push(...jsResults.warnings);
    ...
    const cssResults = checkCSSSemicolons(code);
    errors.push(...cssResults.errors);
    warnings.push(...cssResults.warnings);return { errors, warnings };
}
...
function checkJavaScriptSyntaxFixed(code) {
    const errors = [];
    const warnings = [];// Extract JavaScript from script tags
    const scriptMatches = code.match(/<script[^>]*>([\s\S]*?)<\/script>/gi);
    
    if (!scriptMatches || scriptMatches.length === 0) {return { errors, warnings };
    ...
    const lines = jsCode.split('\n');lines.forEach((line, index) => {
        ...
        const trimmedLine = line.trim();
        const lineNumber = index + 1;// Skip empty lines and comments
        if (!trimmedLine || 
            trimmedLine.startsWith('//') || 
            trimmedLine.startsWith('/*') ||
            trimmedLine.endsWith('*/')) {return;
        }
        ...
        if (trimmedLine.endsWith('{') || 
            trimmedLine === '}' ||
            trimmedLine.startsWith('}') ||
            /^(if|else|for|while|function|try|catch|finally|switch)\s*[\(\{]/.test(trimmedLine)) {return;
        }
        ...
        if (needsSemicolon) {
            const hasSemicolon = trimmedLine.endsWith(';');if (!hasSemicolon) {
...
// Show syntax errors
function showSyntaxErrors(syntaxIssues) {// Debug log
...
function checkAndRestoreCompletion() {
    setTimeout(() => {
        try {
            const isCompleted = localStorage.getItem('partB_lesson12_remake_complete') === 'true';if (isCompleted) {
                const feedback = document.getElementById('feedback');
                const nextBtn = document.getElementById('next-lesson');if (feedback && nextBtn) {
