...
// Enhanced textarea functionality for code editing (your existing function)
...
// Apply the enhancement to your task editor
// Add this after your existing code where taskEditor is defined
...
function validateHTMLStructure(code) {
  ...
  const malformedOpen = /(<[a-zA-Z][^>]*[^>]$)|(<[a-zA-Z][^>]*[^>]\s)/gm;
  if (malformedOpen.test(code)) {
    errors.push('Some HTML tags are missing the closing &gt; character');
  }
...
function validateCSSStructure(code) {
  ...
  // Check for malformed CSS properties (missing colon)
  const malformedProps = /^\s*[a-z-]+\s*[^:;{}]+;/gim;
  if (malformedProps.test(cssCode)) {
    warnings.push('Some CSS properties might be missing colons (:)');
  }
...
document.getElementById('nextLessonBtn').addEventListener('click', function() {
  if (!this.disabled) {
    ...
    // If you have a next lesson, uncomment and modify this line:
    window.location.href = '/2. partB/lesson7_remake.html';
  }
});
