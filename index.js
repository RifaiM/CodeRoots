...
function updateProgress() {
    ...
    topics.forEach(topic => {
        ...
        console.log(`Checking ${topic}: key=${storageKey}, completed=${isCompleted}`);
    ...
    console.log(`Total completed: ${completedCount}/4`);
...
document.getElementById('nextBtnPartB').addEventListener('click', function() {
    if (!this.classList.contains('disabled')) {
        ...
        setTimeout(() => {
            window.location.href = '/2. partB/lesson1_remake.html';
        }, 1000);
    } else {
