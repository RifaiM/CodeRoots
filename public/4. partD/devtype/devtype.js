  ...
  "nextjs": [
    ...
    class DevTypeEngine {
        constructor() {
            ...
            this.lineHeight = 28; // calculated dynamically
        ...
        bindEvents() {
            ...
            // Keyboard Shortcuts (Esc to restart, Tab + Enter)
        ...
        generateContinuousStream() {
            ...
            if (this.timeLimit === 0) {
                // Untimed mode: Choose 1 single random snippet
                return snippets[Math.floor(Math.random() * snippets.length)];
            }

            // Timed mode: Shuffle and chain 20-30 snippets into a long seamless stream
        ...
        resetSession() {
            ...
            this.currentStreamText = this.generateContinuousStream();
        ...
        renderCodeDisplay() {
            ...
            for (let i = 0; i < this.currentStreamText.length; i++) {
                const char = this.currentStreamText[i];
        ...
        handleTyping() {
            ...
            for (let i = 0; i < chars.length; i++) {
                ...
                const expectedChar = this.currentStreamText[i];
            ...
            this.totalTypedChars = val.length;
            this.totalCorrectChars = currentCorrect;

            // Auto-scroll viewport to keep current active character in view
            const currentSpan = this.codeDisplay.querySelector('.char.current');
            if (currentSpan) {
                const spanTop = currentSpan.offsetTop;
                const containerHeight = this.codeDisplay.parentElement ? this.codeDisplay.parentElement.clientHeight : 140;
                // Center cursor when past 2nd line
                if (spanTop > 60) {
                    const scrollOffset = -(spanTop - 45);
                    this.codeDisplay.style.transform = `translateY(${scrollOffset}px)`;
                } else {
                    this.codeDisplay.style.transform = 'translateY(0px)';
                }
            }
            ...
            if (val.length >= this.currentStreamText.length) {
                if (this.timeLimit > 0) {
                    // Extend stream seamlessly if still having time
                    const more = '\n\n' + this.generateContinuousStream();
                    this.currentStreamText += more;
                    
                    const fragment = document.createDocumentFragment();
                    for (let j = val.length; j < this.currentStreamText.length; j++) {
                        const char = this.currentStreamText[j];
                        const span = document.createElement('span');
                        span.className = 'char';
                        if (j === val.length) span.classList.add('current');
                        span.textContent = char;
                        fragment.appendChild(span);
                    }
                    this.codeDisplay.appendChild(fragment);
        ...
        updateStats() {
            ...
            const wpm = Math.round((this.totalCorrectChars / 5) / elapsedMinutes) || 0;
            const cpm = Math.round(this.totalCorrectChars / elapsedMinutes) || 0;
        ...
        finishSpeedrun() {
            ...
            const finalWpm = Math.round((this.totalCorrectChars / 5) / elapsedMinutes) || 0;
            const finalCpm = Math.round(this.totalCorrectChars / elapsedMinutes) || 0;
            const finalAccuracy = this.totalTypedChars > 0 ? Math.round((this.totalCorrectChars / this.totalTypedChars) * 100) : 100;
